...
class TestOCRTemplateWizard(models.TransientModel):
    ...
    x_res_for_survey = fields.Char(string='Result for survey')
    ...
    def process_test_image(self):
        ...
        if ocr_template_id.process_type == 'words':
            ...
            if template:
               render_template = template.sudo()._render_template("wk_ocr_data_fetch.ocr_fetched_data_word_table",{
                    'data': words_map,
                    'max_row': max_row,
        ...
        if template:
            render_template = template.sudo()._render_template("wk_ocr_data_fetch.ocr_process_result", {
                'image_link': image,
                'result': result,
            })
